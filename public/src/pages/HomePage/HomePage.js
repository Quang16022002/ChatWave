...
const HomePage = () => {
  ...
  return (
    <div className='HomePage'>
      ...
      <div style={{ color: 'rgb(245, 196, 105)))', height: 70, padding: '0 15px', margin: '0 4px', borderBottom: '1px solid rgb(252, 205, 129)' }} className='d-flex justify-content-between align-items-center '>
        <div className='Heade-home-left'>
          ...
          <i
            onClick={() => handleIconClick('wallet')}
              className={`fa-solid fa-wallet ${activeIcon === 'wallet' ? 'activeIcon' : ''}`}></i>
          <i
          
            onClick={() => handleIconClick('message')}
            className={`fa-solid fa-message ${activeIcon === 'message' ? 'activeIcon' : ''}`}></i>
          <i className="fa-solid fa-circle"></i>
          <i
            onClick={() => handleIconClick('group')}
            className={`fa-solid fa-user-group ${activeIcon === 'group' ? 'activeIcon' : ''}`}></i>
          <i
            onClick={() => handleIconClick('user')}
            className={`fa-solid fa-user ${activeIcon === 'user' ? 'activeIcon': ''}`}></i>
        </div>
        <div className='Heade-home-right'>
          <i className="fa-solid fa-folder-open"></i>
          <i
            onClick={() => handleIconClick('tag')}
             className={`fa-solid fa-tag ${activeIcon === 'tag' ? 'activeIcon': ''}`}>
          </i>
